...
mhn._create = function(f)
	table.insert(mhn._callbacks, f)
	return #mhn._callbacks
end

mhn._execute = function(id, ...)
	mhn._callbacks[id](...)
end
...
function Map:fn(callback)
	self.rhs = fmt(":<C-u>lua mhn._execute(%s)<CR>", mhn._create(callback))
	return self
end
	...
	for _, c in ipairs(commands) do
		if type(c) == "string" then
			...
			if type(command) == "function" then
				local fn_id = mhn._create(command)
				command = fmt("lua mhn._execute(%s)", fn_id)
			end
			vim.cmd(
				fmt(
					"autocmd %s %s %s %s",
...
mhn.augroup = function(name, commands, clear)
	clear = clear == nil and false or clear
	vim.cmd("augroup " .. name)
	if clear then
		vim.cmd("autocmd!")
	end
	mhn.autocmd(commands)
	vim.cmd("augroup END")
end
...
mhn.highlight = function(name, opts, clear)
...
mhn.highlight_all = function(hls)
	for _, hl in ipairs(hls) do
		mhn.highlight(unpack(hl))
	end
end

mhn.hi_value = (function()
...
---@return string
mhn.darken_color = function(color, percent)
...
---@param args table
function mhn.command(args)
	local name = args[1]
	local rhs = args[2]
	local nargs = args.nargs
	local types = type(args.types) == "table" and table.concat(args.types, " ") or ""

	if type(rhs) == "function" then
		local fn_id = mhn._create(rhs)
		rhs = fmt("lua mhn._execute(%d%s)", fn_id, nargs and ", <f-args>" or "")
	end

	vim.cmd(fmt("command! -nargs=%s %s %s %s", nargs or 0, types, name, rhs))
end
...
---@param opts table
function mhn.notify(lines, opts)
...
if vim.notify then
	...
	vim.notify = function(message, log_level, _)
		...
		mhn.notify(message, { timeout = 5000, log_level = log_level })
	end
end
